...
#
# Prerequisites:
# 1. Run `make init-backend ENV=test` to create the storage account
...
terraform {
  backend "azurerm" {
    ...
    resource_group_name  = "contoso-test-eastus-rg-tfstate"
    storage_account_name = "contosotesteustfstate"
