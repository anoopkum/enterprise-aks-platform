...
provider "azurerm" {
  features {
    # Key Vault features - Production settings
    key_vault {
      # NEVER purge key vaults in production
      purge_soft_delete_on_destroy    = false
      recover_soft_deleted_key_vaults = true
    }

    # Resource Group features - Production settings
    resource_group {
      # ALWAYS prevent destruction of resource groups with resources in production
      prevent_deletion_if_contains_resources = true
    }

    # Virtual Machine features - Production settings
    virtual_machine {
    ...
    # Log Analytics features - Production settings
    log_analytics_workspace {
      # NEVER permanently delete Log Analytics workspaces in production
